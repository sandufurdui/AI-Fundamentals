...
class Boid:
    ...
    def separation(self, boids):
        ...
        for other in boids:
            if self == other:
                continue
            distance = np.linalg.norm(self.position.values - other.position.values)
        ...
        if np.linalg.norm(steer.values) > 0:
            steer = steer / np.linalg.norm(steer.values)
            steer = steer * self.max_speed
            steer = steer - self.velocity
            steer = np.minimum(steer, self.max_force)
            steer = np.maximum(steer, -self.max_force)
        return steer
    ...
    def alignment(self, boids):
        ...
        if count > 0:
            sum = sum / count
            sum = sum / np.linalg.norm(sum.values)
            sum = sum * self.max_speed
    ...
    def cohesion(self, boids):
        ...
        for b in boids:
            if np.all(b.position != self.position):
                total += b.position
                count += 1

        if count > 0:
            avg_pos = total / count
            desired_velocity = avg_pos - self.position
    ...
    def update(self, boids):
        # Apply the separation and alignment behaviors to adjust the velocity
        separation = self.separation(boids)
        alignment = self.alignment(boids)
        self.velocity = self.velocity + separation + alignment
    ...
    def calm_flocking(self, boids):
        ...
        steering = Vector(*steering.values).norm()
...
def draw(canvas):
    ...
    canvas.draw_image(nebula_image, nebula_info.get_center(), nebula_info.get_size(), [CANVAS_RES[0] / 2, CANVAS_RES[1] / 2], [CANVAS_RES[0], CANVAS_RES[1]])
    canvas.draw_image(debris_image, center, size, (wtime - CANVAS_RES[0] / 2, CANVAS_RES[1] / 2), (CANVAS_RES[0], CANVAS_RES[1]))
    canvas.draw_image(debris_image, center, size, (wtime + CANVAS_RES[0] / 2, CANVAS_RES[1] / 2), (CANVAS_RES[0], CANVAS_RES[1]))
